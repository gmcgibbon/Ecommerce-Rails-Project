...
class GameScraper
  ...
  def get_game query_hash
    ...
    {:name => game_info[0], :genre => game_info[1], :price => game_info[2], 
      :developer => game_info[3], :rating => game_info[4], :box_art => game_info[6]}
  end
    ...
    result_set["results"].each do |rset|
      ...
      begin
        ...
        if rset["name"].pan.include?(game_name.pan) && 
          ...
          if reseller_url.include? "costcapt"
            reseller_url = rset["sitedetails"][0]["url"]                                       
          end
          
          # has result provided all the needed values?
          break unless name.empty? or price.to_s.empty? or genre.empty? or 
                       rating.empty? or developer.empty? or reseller_url.empty?
        end
      rescue #=> err
